...
each backward pass we apply gradient clipping at $\ell_2$-norm $5$ jointly
to both flows; the clip is rarely active at convergence but eliminates
the rare exploding-gradient events that occur in the first few epochs
when the implicit gradient in~\eqref{eq:implicit} is evaluated near a
still-flat $h'_\theta$. We early-stop on the training objective with
patience $35$--$50$ and a relative tolerance of $10^{-4}$, restoring the
best-loss state at the end of training. Total training time is
approximately $20$ seconds per Monte Carlo replication on a single CPU
core at $n_{\text{train}} = 384$, scaling near-linearly to roughly $300$
seconds at $n_{\text{train}} = 2048$; convergence typically occurs around
epoch $130$--$400$ depending on the regime and is uniform across seeds.
