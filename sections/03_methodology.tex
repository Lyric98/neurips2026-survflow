...
Under the restricted parameter space $\Theta^\dagger$
(Appendix~\ref{app:identifiability}), the \emph{marginal distribution of
$T$} is identifiable from right-censored data: if two parameter settings
produce the same distribution over the observable $(\min(T, C), \delta, Z)$,
they induce the same conditional distribution $f_{T \mid Z}$.
\end{quote}
